...
class lmmNet_beta:
    ...
    def __init__(self, h, X, M, scheme, hidden_units, loss_type = 'L1'):
        """
        Args:
        h -- step size
        X -- data array with shape S x N x D 
        ...
        """
        self.loss_type = loss_type # either L1, L2, or Wasserstein
    ...
    def get_Y(self, X):
        ...
        # CHANGED HERE: L1- error
        if self.loss_type == 'L1':
            difference = self.D * tf.reduce_mean(tf.math.abs(Y))
        elif self.loss_type == 'L2':
            difference = self.D * tf.reduce_mean(tf.square(Y))
        elif self.loss_type == 'Wasserstein':
            u = self.alpha[0] * X[:, M: ,:]
            for m in range(1, M+1):
                u += self.alpha[m] * X[:,M-m:-m,:]
            v = self.h * self.beta[0] * self.get_F(X[:, M:, :])
            for m in range(1, M+1):
                v += self.h * self.beta[m] * self.get_F(X[:, M-m:-m, :])
            difference = wasserstein_distance(tf.reshape(u, [-1]), tf.reshape(v, [-1]))
            difference = tf.convert_to_tensor(difference)
        else:
            raise Exception("Loss type is unknown. Please choose L1, L2, or Wasserstein.")
